...
Description automatically generatedThe Create New Project appears. The first option is to select the type of activity. Choose Empty, which is the default and click Next. 
...
Under the action column, click on the Start Android Virtual Device icon to run the AVD. The Android Emulator uses the AVD to mimic the device.  You can then use the control panel to manage the device. The Extend control button at the bottom gives you more options 
